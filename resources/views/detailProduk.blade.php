        ...
        <div class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div
                        class="p-5 sm:p-8 md:p-10 bg-white shadow-sm border border-gray-100 rounded-[1.5rem] sm:rounded-[2rem]">
                        <span
                        ...
                        <h1
                            class="text-xl sm:text-3xl md:text-4xl font-extrabold text-gray-900 leading-tight mb-3 sm:mb-4">
                            {{ $produk->nama_produk }}
                        </h1>

                        <div class="mb-5 sm:mb-8 mt-3 sm:mt-6">
                            <p class="text-xs sm:text-sm text-gray-500 font-bold mb-1">Harga mulai dari</p>
                            <h2 class="text-2xl sm:text-4xl md:text-5xl font-extrabold text-[#4F46E5]">
                                Rp{{ number_format($produk->harga, 0, ',', '.') }}</h2>
                        </div>

                        <div>
                            <h3 class="text-base sm:text-lg font-extrabold text-gray-900 mb-3 sm:mb-4">Deskripsi</h3>
                            <div
                                class="prose prose-sm text-gray-600 font-medium space-y-3 sm:space-y-4 whitespace-pre-line text-sm">
                                {{ $produk->deskripsi }}
                            </div>
                        </div>
                    </div>
                    ...
                    @if(!empty($portfolio))
                        <div class="pt-2 sm:pt-4">
                            <span
                                class="text-[#4F46E5] font-bold text-xs sm:text-sm uppercase tracking-wider">Portfolio</span>
                            ...
                            <div class="flex flex-col gap-3">
                                @foreach($portfolio as $item)
                                    <a href="{{ $item['link'] ?? '#' }}" target="_blank" rel="noopener noreferrer"
                                        ...
                                        <div
                                            class="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-indigo-100 transition-colors">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-bold text-gray-900 truncate">
                                                {{ $item['judul'] ?? 'Link Karya' }}</p>
                                            <p class="text-xs text-indigo-500 font-medium truncate mt-0.5">
                                                {{ $item['link'] ?? '' }}</p>
                                        </div>
                                        <svg class="w-4 h-4 text-gray-300 shrink-0 group-hover:text-indigo-400 transition-colors"
                ...
                <div class="lg:col-span-1">
                    ...
                    <div
                        class="p-5 sm:p-6 bg-white shadow-sm border border-gray-100 rounded-[1.5rem] sm:rounded-[2rem] lg:sticky lg:top-8">
                        ...
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 font-medium">
                                    {{ $produk->user->sellerProfile->bidang_keahlian ?? 'Seller' }}</p>
                            </div>
                        </div>
